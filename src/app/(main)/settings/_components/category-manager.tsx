...
export function CategoryManager() {
  return (
    <div className="border border-gray-300 rounded-lg p-3">
      <span className="text-xs font-mono text-gray-400 uppercase tracking-wider">카테고리 관리</span>
      <div className="mt-2 space-y-1.5">
        {CATEGORIES.map((cat) => (
          ...
          >
            ...
            <span className="text-xs text-red-400 cursor-pointer hover:text-red-600">삭제</span>
          </div>
        ))}
      </div>
      <div className="mt-2">
        <div className="bg-blue-500 text-white px-2 py-1 text-xs rounded-md inline-flex cursor-pointer hover:bg-blue-600">
          + 카테고리 추가
        </div>
      </div>
    </div>
